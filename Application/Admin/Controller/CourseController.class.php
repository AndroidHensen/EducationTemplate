...
//        $this->assign('positions', $position);
        ...
        $news = D("Course")->getNews($cond, $page, $pageSize);
        $count = D("Course")->getNewsCount($cond);
    ...
    public function add()
    {
        if ($_POST) {
            if (!isset($_POST['title']) || !$_POST['title']) {
                return show(0, "教师姓名不存在");
            }
            if (!isset($_POST['small_title']) || !$_POST['small_title']) {
                return show(0, "教师职称不存在");
            }
...
//            $this->assign('copyfrom', $copyFrom);
    ...
    public function edit()
    {
        ...
        if (!$newsId) {
            //执行跳转
            $this->redirect('/admin.php?c=course');
        }
        $news = D("Course")->find($newsId);
        if (!$news) {
            //执行跳转
            $this->redirect('/admin.php?c=course');
        }
...
//        $this->assign('copyfrom', C("COPY_FROM"));
    ...
    public function save($data)
    {
        ...
        unset($data['news_id']);
        try {
            $id = D("Course")->updateById($newsId, $data);
    ...
    public function setStatus()
    {
        try {
            if ($_POST) {
                ...
                $res = D("Course")->updateStatusById($id, $status);
                if ($res) {
                    return show(1, '操作成功');
                } else {
                    return show(0, '操作失败');
        ...
        if ($listorder) {
            try {
                foreach ($listorder as $menuId => $v) {
                    $id = D('Course')->updateNewsListorderById($menuId, $v);
                    //记录错误信息
                    if ($id === false) {
                        $errors[] = $menuId;
                    }
