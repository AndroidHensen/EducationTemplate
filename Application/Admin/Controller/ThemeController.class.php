...
//        $this->assign('positions', $position);
        ...
        $news = D("Theme")->getNews($cond, $page, $pageSize);
        $count = D("Theme")->getNewsCount($cond);
...
//            $this->assign('copyfrom', $copyFrom);
    ...
    public function edit()
    {
        ...
        if (!$newsId) {
            //执行跳转
            $this->redirect('/admin.php?c=theme');
        }
        $news = D("Theme")->find($newsId);
        if (!$news) {
            //执行跳转
            $this->redirect('/admin.php?c=theme');
        }
...
//        $this->assign('copyfrom', C("COPY_FROM"));
    ...
    public function save($data)
    {
        ...
        unset($data['news_id']);
        try {
            $id = D("Theme")->updateById($newsId, $data);
    ...
    public function setStatus()
    {
        try {
            if ($_POST) {
                ...
                $res = D("Theme")->updateStatusById($id, $status);
                if ($res) {
                    return show(1, '操作成功');
                } else {
                    return show(0, '操作失败');
        ...
        if ($listorder) {
            try {
                foreach ($listorder as $menuId => $v) {
                    $id = D('Theme')->updateNewsListorderById($menuId, $v);
                    //记录错误信息
                    if ($id === false) {
                        $errors[] = $menuId;
                    }
