...
//        $this->assign('positions', $position);
        ...
        $news = D("Teach")->getNews($cond, $page, $pageSize);
        $count = D("Teach")->getNewsCount($cond);
...
//            $this->assign('copyfrom', $copyFrom);
    ...
    public function edit()
    {
        ...
        if (!$newsId) {
            //执行跳转
            $this->redirect('/admin.php?c=teach');
        }
        $news = D("Teach")->find($newsId);
        if (!$news) {
            //执行跳转
            $this->redirect('/admin.php?c=teach');
        }
...
//        $this->assign('copyfrom', C("COPY_FROM"));
    ...
    public function save($data)
    {
        ...
        unset($data['news_id']);
        try {
            $id = D("Teach")->updateById($newsId, $data);
    ...
    public function setStatus()
    {
        try {
            if ($_POST) {
                ...
                $res = D("Teach")->updateStatusById($id, $status);
                if ($res) {
                    return show(1, '操作成功');
                } else {
                    return show(0, '操作失败');
        ...
        if ($listorder) {
            try {
                foreach ($listorder as $menuId => $v) {
                    $id = D('Teach')->updateNewsListorderById($menuId, $v);
                    //记录错误信息
                    if ($id === false) {
                        $errors[] = $menuId;
                    }
