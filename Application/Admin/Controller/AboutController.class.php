...
//        $this->assign('positions', $position);
        ...
        $news = D("About")->getNews($cond, $page, $pageSize);
        $count = D("About")->getNewsCount($cond);
...
//            $this->assign('copyfrom', $copyFrom);
    ...
    public function edit()
    {
        ...
        if (!$newsId) {
            //执行跳转
            $this->redirect('/admin.php?c=about');
        }
        $news = D("About")->find($newsId);
        if (!$news) {
            //执行跳转
            $this->redirect('/admin.php?c=about');
        }
...
//        $this->assign('copyfrom', C("COPY_FROM"));
    ...
    public function save($data)
    {
        ...
        unset($data['news_id']);
        try {
            $id = D("About")->updateById($newsId, $data);
...
//            if ($id === false || $condId === false) {
    ...
    public function setStatus()
    {
        try {
            if ($_POST) {
                ...
                $res = D("About")->updateStatusById($id, $status);
                if ($res) {
                    return show(1, '操作成功');
                } else {
                    return show(0, '操作失败');
        ...
        if ($listorder) {
            try {
                foreach ($listorder as $menuId => $v) {
                    $id = D('About')->updateNewsListorderById($menuId, $v);
                    //记录错误信息
                    if ($id === false) {
                        $errors[] = $menuId;
                    }
    ...
    public function push()
    {
        ...
        try {
            $news = D("About")->getNewsByNewsIdIn($newId);
            if (!$news) {
